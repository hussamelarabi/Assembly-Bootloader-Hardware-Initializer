...
;**************************** Padding and Signature **********************************
   ;partition entry 1 (MBR)
      times 446-($-$$) db 0   ;padding of zeroes from current address to the 446th byte
      t1:
      db 0x80                 ;byte indicating a bootable partition
      db 0x00, 0x01, 0x00     ;Starting CHS of (0,1,1)
      db 0x83                 ;FAT32 format
      db 0x80, 10111111b, 0x00     ;Ending CHS
      dd 0x01      ;relative LBA index of 1 (since first sector is reserved for the MBR)
                              ;0xF007FE is hexadecimal representation of 15730686 which is the max number of sectors my specific usb hold minus 2 (minus 1 since index starts at zero and minus one to account for the MBR)
      dd  0xF4200
      t2:
            dq 0x0,0x0
      t3:
      ...
      ;rest of the entries are zeroed out by the already given instruction below
